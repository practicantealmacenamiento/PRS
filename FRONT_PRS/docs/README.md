# Documentacion Frontend RadioFrecuencias

Esta carpeta consolida la documentacion funcional y tecnica del frontend de **RadioFrecuencias**, la aplicacion web que permite registrar prestamos, devoluciones, historicos y la administracion de catalogos a traves de una interfaz Next.js. El frontend consume la API REST del backend Django (`/api/`) y maneja autenticacion JWT, refresco de tokens, exportacion de reportes y un panel administrativo centralizado.
...
> Nota: esta documentacion cubre exclusivamente el frontend Next.js. La documentacion del backend Django se mantiene en `BACK_PRS/docs/`.
